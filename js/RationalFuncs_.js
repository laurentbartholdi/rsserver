...
function makeEvalPoly(argN, argD, resName, resNameX, resNameY, resNameZ, coefs, degree)
{
	...
	res += "+ complexMul(" + getComplexString(coefs[j]) + ", " + argN + j + argD + (degree-1-j) + ")";
    res += ";\n";
    res += resNameY + " = ";
    for (var j = 1; j <= degree-1; j++)
	res += "+ complexMul(" + getComplexString(coefs[j].clone().multiplyScalar(j)) + ", " + argN + (j-1) + argD + (degree-1-j) + ")";
    res += ";\n";
...
"}";
var complexToColorStringL = 	"vec3 complex2rgb(vec2 n, vec2 d){\n"+ // n/d is a normalized P1 point
